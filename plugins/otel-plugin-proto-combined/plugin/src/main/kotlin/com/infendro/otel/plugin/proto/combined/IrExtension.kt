...
class IrExtension(
    ...
    override fun generate(
        ...
        val TracerProviderBuilder_setSampler = TracerProviderBuilder.getFunction("setSampler")
        val TracerProviderBuilder_build = TracerProviderBuilder.getFunction("build")

        val Sampler = getClass("io.opentelemetry.kotlin.sdk.trace.samplers", "Sampler")
        val SamplerCompanion = Sampler.getClass("Companion")
            ...
            initializer = expression {
                call(TracerProviderBuilder_build) {
                    dispatchReceiver = call(TracerProviderBuilder_addSpanProcessor) {
                        dispatchReceiver = call(TracerProviderBuilder_setSampler) {
                            dispatchReceiver = call(TracerProviderCompanion_builder) {
                                dispatchReceiver = irGetObject(TracerProviderCompanion)
                            }
                            argument(0, call(SamplerCompanion_alwaysOn) {
                                dispatchReceiver = irGetObject(SamplerCompanion)
                            })
                        }
