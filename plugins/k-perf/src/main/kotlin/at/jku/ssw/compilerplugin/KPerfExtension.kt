                ...
                override fun visitFunctionNew(declaration: IrFunction): IrStatement {
                  ...
                  if (declaration.name.asString() == "_enter_method" ||
                                  ...
                                  (!instrumentPropertyAccessors &&
                                          declaration.origin == DEFAULT_PROPERTY_ACCESSOR) ||
                                  declaration.fqNameWhenAvailable?.asString()?.contains("<init>") !=
                                          false ||
                                  declaration
                                          .fqNameWhenAvailable
                                          ?.asString()
                                          ?.contains("<anonymous>") != false ||
                                  (declaration.fqNameWhenAvailable?.asString()?.let { fqn ->
                                    methodRegexes.none { it.matches(fqn) }
                                  } == true)
                  ) {
                    // do not further transform this method, e.g., its statements are not
                    // transformed
                    println(
                            "# Do not wrap body of ${declaration.name} (${declaration.fqNameWhenAvailable?.asString()}):\n${declaration.dump()}"
                    )
                    return declaration
                  }
                  println(
                          "# Wrap body of ${declaration.name} (${declaration.fqNameWhenAvailable?.asString()}, origin: ${declaration.origin})):\n${declaration.dump()}"
                  )
