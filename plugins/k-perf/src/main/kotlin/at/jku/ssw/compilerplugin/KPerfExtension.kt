...
class KPerfExtension(
  ...
  fun printText(str: String) {
    ...
    // Do not append [kpref] in file since we are the only ones writing there
    debugFile.appendText(str)
  }

  private fun shouldInstrument(declaration: IrFunction): Boolean {
    val name = declaration.name.asString()
    val fqName = declaration.fqNameWhenAvailable?.asString()

    if (name == "_enter_method") {
      printText("SKIP $name: synthetic k-perf function")
      return false
    }
    if (name == "_exit_method") {
      printText("SKIP $name: synthetic k-perf function")
      return false
    }
    if (name == "_exit_main") {
    ...
    if (declaration.body == null) {
      printText("SKIP $name: no body")
      return false
    }
    if (declaration.origin == ADAPTER_FOR_CALLABLE_REFERENCE) {
      printText("SKIP $name: adapter for callable reference")
      return false
    }
    if (!instrumentPropertyAccessors && declaration.origin == DEFAULT_PROPERTY_ACCESSOR) {
      printText("SKIP $name: property accessor (instrumentPropertyAccessors=false)")
      return false
    }
    if (fqName == null) {
      printText("SKIP $name: no fully-qualified name available")
      return false
    }
    if (fqName.contains("<init>")) {
      printText("SKIP $name ($fqName): constructor")
      return false
    }
    if (fqName.contains("<anonymous>")) {
      printText("SKIP $name ($fqName): anonymous function")
      return false
    }
    if (methodRegexes.none { it.matches(fqName) }) {
      printText("SKIP $name ($fqName): does not match any method regex")
      return false
    }
    return true
  }
  ...
  override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
    ...
    val stringBuilderClass =
      stringBuilderTypeAlias?.owner?.expandedType?.classOrFail
        ?: pluginContext.referenceClass(
          stringBuilderClassId
    ...
    val stringBuilderAppendIntFunc =
      stringBuilderClass.functions.single {
        it.owner.name.asString() == "append" &&
                it.owner.hasShape(true, regularParameters = 1) &&
    ...
    val pathConstructionFunc =
      pluginContext.referenceFunctions(
        CallableId(FqName("kotlinx.io.files"), Name.identifier("Path"))
      )
    ...
    val stringBuilder: IrField =
      pluginContext.irFactory
        ...
        .apply {
          this.initializer =
            DeclarationIrBuilder(pluginContext, firstFile.symbol)
              .irExprBody(
                DeclarationIrBuilder(pluginContext, firstFile.symbol)
                  .irCallConstructor(
    ...
    val randomNumber =
      pluginContext.irFactory
        ...
        .apply {
          initializer =
            DeclarationIrBuilder(pluginContext, firstFile.symbol).run {
              irExprBody(
                irCall(nextIntFunc).apply {
            ...
            DeclarationIrBuilder(pluginContext, firstFile.symbol).run {
              irExprBody(
                irConcat().apply {
                  addArgument(
                    irString(
                      "./trace_${pluginContext.platform!!.presentableDescription}_"
                    )
                  )
      ...
      pluginContext.irFactory
        ...
        .apply {
          initializer =
            DeclarationIrBuilder(pluginContext, firstFile.symbol).run {
              irExprBody(
                irCall(bufferedFunc).apply {
                  arguments[0] =
                    irCall(sinkFunc).apply {
                      arguments[0] =
            ...
            DeclarationIrBuilder(pluginContext, firstFile.symbol).run {
              irExprBody(
                irConcat().apply {
                  addArgument(
                    irString(
                      "./symbols_${pluginContext.platform!!.presentableDescription}_"
                    )
                  )
                  // TODO: use kotlinx.datetime.Clock.System.now()
                  addArgument(irGetField(null, randomNumber))
                ...
                +irReturn(
                  irCall(funMarkNow).also { call ->
                    call.dispatchReceiver =
                      irGetObject(timeSourceMonotonicClass)
                  }
                )
              }
        }
            ...
            DeclarationIrBuilder(pluginContext, symbol, startOffset, endOffset)
              .irBlockBody {
                ...
                val elapsedMicrosProp: IrProperty =
                  elapsedDuration.type.getClass()!!.properties.single {
    ...
    fun buildMainExitFunction(): IrSimpleFunction {
      fun IrBlockBodyBuilder.writeAndFlushSymbolsFile() {
        ...
        +irCall(flushFunc).apply { dispatchReceiver = irGetField(null, bufferedSymbolsFileSink) }
      }

      fun IrBlockBodyBuilder.printFileNamesToStdout() {
        +irCall(printlnFunc).apply { arguments[0] = irGetField(null, bufferedTraceFileName) }
        +irCall(printlnFunc).apply { arguments[0] = irGetField(null, bufferedSymbolsFileName) }
      }
      ...
      return DeclarationIrBuilder(pluginContext, func.symbol).irBlockBody {
        ...
        val startTime =
          irTemporary(
            irCall(enterFunc).apply {
              arguments[0] =
                methodIdMap[func.kotlinFqName.asString()]!!.toIrConst(
        ...
        +irTry(
          ...
          if (func.name.asString() == "main")
            irCall(exitMainFunc).apply { arguments[0] = irGet(startTime) }
          else irCall(exitFunc).apply { arguments[0] = irGet(startTime) }
        )
      }
    }
      ...
      file.transform(
        object : IrElementTransformerVoidWithContext() {
          override fun visitFunctionNew(declaration: IrFunction): IrStatement {
            ...
            val fqName = declaration.fqNameWhenAvailable!!.asString()
            printText("WRAP body of ${declaration.name} ($fqName, origin: ${declaration.origin})):\n${declaration.dump()}")
            instrumentedFunctions.add(fqName)
