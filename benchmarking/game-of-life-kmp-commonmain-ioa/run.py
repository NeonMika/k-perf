...
from build import (
    IoaConfig,
    clean_game_of_life_commonmain_ioa,
...
def _parse_args() -> Args:
    ...
    parser.add_argument("--no-ioa", dest="ioa", action="store_false")
    ...
    parser.add_argument("--ioa-kinds", nargs="*", default=IOA_KINDS)
...
def _build_ioa_configs(args: Args) -> List[IoaConfig]:
    return [IoaConfig(kind) for kind in args.ioa_kinds]
...
def _clean_if_needed(args: Args):
    ...
    _print_header("# Cleaning IOA benchmark dependencies...")
    clean_kir_helperkit()
    clean_instrumentation_overhead_analyzer_plugin()
    clean_game_of_life_commonmain_ioa()


def _build_all(args: Args, configs: List[IoaConfig]) -> Dict[str, float]:
    build_times: Dict[str, float] = {}

    _print_header("# Building IOA benchmark dependencies...")
    ...
    for cfg in configs:
        build_times.update(build_game_of_life_commonmain_ioa_variant(cfg))
...
def _collect_executables_and_verify(
    ...
    plain_root = ROOT_DIR / "kmp-examples" / "game-of-life-kmp-commonmain" / "build"
    ioa_root = ROOT_DIR / "kmp-examples" / "game-of-life-kmp-commonmain-ioa" / "build"

    # Reference versions
    if args.reference and args.jvm:
        _verify(Executable(
            name="commonmain_plain_jar",
            path=plain_root / "lib" / f"game-of-life-kmp-commonmain-jvm-{PROJECT_VERSION}.jar",
            kind="jar",
            config=None,
        ))
    ...
    if args.reference and args.js:
        _verify(Executable(
            ...
            path=plain_root / "js" / "packages" / "game-of-life-kmp-commonmain" / "kotlin" / "game-of-life-kmp-commonmain.js",
            kind="node",
            config=None,
        ))
    ...
    if args.reference and args.native:
        ...
        for target in native_targets:
            exe = Executable(
                name="commonmain_plain_native",
                path=plain_root / "bin" / target / "releaseExecutable" / f"game-of-life-kmp-commonmain{native_ext}",
                kind="native",
                config=None,
            )
            if _verify(exe, fail_soft=True):
                ...
                break
        if not found:
            print(f"ERROR: NOT FOUND commonmain_plain_native at any of {native_targets}")
            missing.append(exe)
    ...
    if args.ioa:
        for cfg in configs:
            ...
            if args.jvm:
                _verify(Executable(
                    name=f"commonmain_ioa_{suffix}-jar",
                    path=ioa_root / "lib" / f"game-of-life-kmp-commonmain-ioa-jvm-{PROJECT_VERSION}-{suffix}.jar",
                    kind="jar",
                    config=cfg,
                ))
            ...
            if args.js:
                _verify(Executable(
                    name=f"commonmain_ioa_{suffix}-node",
                    path=ioa_root / "js" / "packages" / f"game-of-life-kmp-commonmain-ioa-{suffix}" / "kotlin" / f"game-of-life-kmp-commonmain-ioa-{suffix}.js",
                    kind="node",
                    config=cfg,
                ))
            ...
            if args.native:
                ...
                for target in native_targets:
                    exe = Executable(
                        name=f"commonmain_ioa_{suffix}-native",
                        path=ioa_root / "bin" / target / "releaseExecutable" / f"game-of-life-kmp-commonmain-ioa-{suffix}{native_ext}",
                        kind="native",
                        config=cfg,
                    )
                    if _verify(exe, fail_soft=True):
                        ...
                        break
                if not found:
                    print(f"ERROR: NOT FOUND commonmain_ioa_{suffix}-native at any of {native_targets}")
                    missing.append(exe)
...
def _timestamped_measurement_dir() -> Path:
    ...
    name = f"{ts}_game-of-life-kmp-commonmain-ioa"
...
def main():
    ...
    if not (args.reference or args.ioa):
        raise ValueError("At least one of --reference or --ioa must be true")
    if not (args.jvm or args.js or args.native):
        raise ValueError("At least one of --jvm, --js, or --native must be true")
    if not args.ioa_kinds:
        raise ValueError("--ioa-kinds must contain at least one value")
    ...
    configs = _build_ioa_configs(args)
    _print_header("# IOA Configurations to Build")
    for cfg in configs:
        print(f"- {cfg.suffix()}")
