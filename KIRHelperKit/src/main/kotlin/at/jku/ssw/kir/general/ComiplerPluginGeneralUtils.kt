package at.jku.ssw.kir.general
...
import at.jku.ssw.kir.find.irpropertysymbol.findFunction
...
class IrStringBuilder(
  ...
  fun append(value: Any): IrFunctionAccessExpression {
    ...
    return DeclarationIrBuilder(
      pluginContext,
      stringBuilderField.symbol,
      stringBuilderField.startOffset,
      stringBuilderField.endOffset
    ).getCall(pluginContext) {
   ...
   */
  fun delete(start: Int, end: Int): IrFunctionAccessExpression = DeclarationIrBuilder(
    pluginContext,
    stringBuilderField.symbol,
    stringBuilderField.startOffset,
    stringBuilderField.endOffset
   ...
   */
  fun irToString(): IrFunctionAccessExpression = DeclarationIrBuilder(
    pluginContext,
    stringBuilderField.symbol,
    stringBuilderField.startOffset,
    stringBuilderField.endOffset
...
class IrFileReader(
  ...
  private val readStringFunction = pluginContext.findFunction(
    "kotlinx/io/readString()",
    extensionReceiverType = pluginContext.findClass("kotlinx/io/Source").defaultType
  )
...
class IrFileWriter(
  ...
  fun writeData(data: Any): IrFunctionAccessExpression {
    require(extractFQTypeNameFromIrNode(data) == "string") { "Write function only supports string types" }
