...
class GameOfLife(
  ...
  fun nLiveNeighbors(row: Int, col: Int): Int {
    return (-1..1)
            .flatMap { dx ->
              (-1..1).map { dy ->
                if (dx == 0 && dy == 0) 0
                else if (isValidPosition(row + dx, col + dy) && field[row + dx][col + dy]) 1 else 0
              }
            }
            .sum()
  }
  ...
  fun step() {
    val newField = Array(height) { row -> Array(width) { col -> shouldBecomeLive(row, col) } }
    field = newField
  }
